...
import { IArticleUseCases } from "./interfaces/IArticleUseCase";
...
export class ArticleUseCases implements IArticleUseCases {
  ...
  async updateLikesDislikes(
    ...
    const updatedArticle = await this.articleRepository.updateLikesDislikes(
      articleId,
      userId,
      action
    );
  ...
  async unblockArticle(articleId: string, userId: string): Promise<IArticle> {
    ...
    const updatedBlocks = article.blocks.filter(
